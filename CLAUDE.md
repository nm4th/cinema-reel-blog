...
**venue photo の追加配置**: iframe だけでは視覚密度が薄い場合（地味な題材で公式動画が 1〜2 本しか見つからない等）、CINEMA REEL 新宿の実空間写真（Spacemarket CDN）を `<figure>` で挿入する。これは「作品画像の hotlink/転載」とは違って、**ユーザー所有のスペース写真** なので合法。視聴方法・選択肢・グループ視聴系のセクションに 1〜2 枚入れるのが標準。

**lint 最低ライン**:
- 上映ガイド: iframe ≥ 2、iframe + figure 合計 ≥ 3
