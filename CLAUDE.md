...
| 用途 | やること | NG |
|---|---|---|
| Hero画像 | DALL-E自前生成（image-manifest.json + Actions） | 公式キービジュアルの hotlink・転載 |
| 作品サムネ | YouTube公式チャンネルの予告編を `<div class="video-embed"><iframe>` で埋め込み | 静止画のキャプチャを保存して再アップ |
| 出演者写真 | 使わない、テキストリンクのみ | 事務所・公式画像の転載 |
| 出典 | テキストリンク（公式サイト・公式X・公式YouTube） | 「画像の引用URL」と称した hotlink |
...
2. **H2 #1: 作品概要** — 配信開始日（ソースリンク付き）/ 原作 / 全話数 / 制作陣
3. **H2 #2: メインキャスト** — 役名+俳優名、ソースは公式 about.netflix.com など
4. **H2 #3: 公式予告編** — `<div class="video-embed">` で埋め込み
5. **H2 #4: 大画面で観たい人向けの選択肢** — 自宅 / 友人宅プロジェクター / プライベートシネマ。ここで CINEMA REEL を1回さりげなく
6. **H2 #5: グループ視聴の楽しみ方** — 一気観 vs 分割視聴の提案、グッズ持ち寄り、感想戦の進め方など
7. **公式情報まとめ**（`<div class="source-list">`）: 公式サイト・公式予告編・関連プレスリリースのリンク一覧
8. **クロージング**（1〜2文）: 軽く CINEMA REEL を選択肢として再登場させる
